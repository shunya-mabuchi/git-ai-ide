...
2026-05-11 時点では、demo repo を使って Branch Goal から Patch Queue、Diff Preview、Runtime checks、Local Preview preflight、commit draft、branch push demo、PR creation demo まで一通り説明できます。

実装済み:

- VS Code / Cursor 風の IDE layout
- Explorer folder tree、editor tabs、search、source control
- Monaco Editor / Monaco Diff Editor
- Branch Goal と Assisted Memory の editable Markdown
- Context budget と task priority
- Recorded AI / WebLLM / Ollama の runtime routing
- Ollama Patch Proposal の E2E 診断 UI
- Structured Edit Operation validation
- Patch Queue と Safety Checklist
- Local Preview の WebContainer preflight
- PR 作成前 Safety Gate
- GitHub PR Flow readiness checklist
- Cloudflare Worker / D1 の API boundary

まだ「実 credentials / 実 runtime での確認」が残るもの:

- GitHub App credentials を Cloudflare Worker に設定した状態での selected repo E2E
- WebLLM の実モデルロード
- Ollama が起動している環境での実 Patch Proposal
- WebContainer 対応 repo での iframe preview E2E

ただし、これらは demo mode / recorded fallback / preflight / diagnostic log として UI 上で明示しています。面接では「できている」と言い切るのではなく、「実 runtime に差し替えられる境界を作り、未接続時も説明可能な fallback にしている」と説明します。

## 技術選定を聞かれたときの短い答え

React / Vite / TypeScript:
ブラウザ IDE は Monaco、WebContainer、WebGPU、IndexedDB など client-side の統合が中心なので、SSR より SPA が合っています。TypeScript で AI output、Git state、Patch Proposal の境界を型として固定しました。

Cloudflare Pages / Workers / D1:
無料公開を継続しやすく、静的 UI、GitHub API proxy、workflow metadata DB を同じ Cloudflare 上で説明できます。D1 は code 保存ではなく metadata 保存に限定しています。

WebLLM + Ollama fallback:
公開後に cloud LLM API cost を持たないためです。ブラウザ内 LLM は制約があるので、Context Pack と task priority で小さな task に分解し、重い処理は Ollama fallback に逃がせる設計にしています。

WebContainer:
実際の IDE として、変更後に test / typecheck / preview へ進める価値を出すためです。全 repo で動くとは言わず、preflight checklist で対応可否を表示します。

GitHub App:
selected repository only の権限にできるためです。PAT や OAuth App よりも、ポートフォリオで security posture を説明しやすいです。

## 面接で強く言えるポイント

- 小さい LLM の限界を前提に、AI に渡す文脈と出力形式を制御した
- AI の出力を直接ファイルに反映せず、structured edit、schema validation、diff review を必須にした
- GitHub App / Worker / D1 の境界で、ブラウザに secret を置かない構成にした
- WebContainer や WebLLM のような不安定になりやすい runtime を、preflight と fallback で説明可能にした
- demo mode を逃げではなく、未設定環境でも価値を伝える first-class path として扱った
